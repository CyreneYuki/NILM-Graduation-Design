...
parser.add_argument('--app', type=str, default='dishwasher',
                    help='washingmachine, fridge, dishwasher, microwave')
...
# dont use pretrain model
encoder = AggEncoder().to(device)
decoder = AggDecoder().to(device)
decoder = nn.Sequential(decoder, torch.nn.Linear(opt.window_len, opt.window_len).to(device))

# use pretrain model
# encoder = torch.load(opt.save_dir + '/'+opt.dataset+'/AggEncoder.pth').to(device)
# decoder = torch.load(opt.save_dir + '/'+opt.dataset+'/AggDecoder.pth').to(device)
# decoder = nn.Sequential(decoder, torch.nn.Linear(opt.window_len, opt.window_len).to(device))    # 加一层全连接作为分类器
...
'loss function'
ae_loss = torch.nn.BCEWithLogitsLoss().to(device)
...
for epoch in range(opt.n_epochs):
    ...
    if val_loss <= best_loss:
        ...
        best_loss = val_loss
        best_epoch = epoch
        if not os.path.exists(opt.save_dir + '/'+opt.dataset+ '/'+opt.app):  # 判断当前路径是否存在，没有则创建new文件夹
            os.makedirs(opt.save_dir + '/'+opt.dataset+ '/'+opt.app)
        torch.save(encoder, opt.save_dir + '/'+opt.dataset+ '/'+opt.app+'/Encoder.pth')
        torch.save(decoder, opt.save_dir + '/'+opt.dataset+ '/'+opt.app+'/Decoder.pth')
