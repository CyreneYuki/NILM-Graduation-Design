...
from run_model import run_model_pretrain
...
'loss function'
ae_loss = torch.nn.SmoothL1Loss().to(device)
...
for epoch in range(opt.n_epochs):
    ...
    train_loss = run_model_pretrain(model=ae,
    ...
    val_loss = run_model_pretrain(model=ae,
    ...
    if val_loss <= best_loss:
        ...
        torch.save(encoder, opt.save_dir + '/'+opt.dataset+ '/AggEncoder.pth')
        torch.save(decoder, opt.save_dir + '/'+opt.dataset+ '/AggDecoder.pth')
