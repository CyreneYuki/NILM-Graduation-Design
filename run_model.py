...
def run_model_pretrain(model, iterator, optimizer, criterion, device, if_train=True):
    model.eval()
    if if_train:
        model.train()
...
def test_model_ae(model, iterator, device):
    ...
    with torch.no_grad():
        for i, (agg, app, app_onoff) in enumerate(progress_bar):
            ...
            truth_list.append(truth_slice.reshape(-1, 1))
            pred_list.append(pred_slice.reshape(-1, 1))
            truth_onoff_list.append(truth_onoff_slice.reshape(-1, 1))
